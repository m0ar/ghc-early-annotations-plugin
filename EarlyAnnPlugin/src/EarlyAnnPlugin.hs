module EarlyAnnPlugin (plugin) where
  ...
  -- 'HsParsedModule' and adds them to the typechecking global
  -- environment 'TcGblEnv', for use later in the compilation
  -- pipeline.
  --
  ...
  findAnnDecls hpm = let L _ hsModule = hpm_module hpm in
    let lHsDecls = hsmodDecls hsModule in
      stripAndFilter lHsDecls
